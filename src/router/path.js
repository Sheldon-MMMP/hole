//tabbar
export const homePath = '/home';
export const giftPath = '/gift';
export const userPath = '/user';

export const errPath = '/error'

// order
export const orderListPath = '/order/list'
export const orderMenuPath = '/order/select'

export const moneyRecharge = "/money/recharge"
export const orderSuccess = '/order/success'

// clerk
export const clerkDetailPath = '/clerk/detail'

export const searchPagePath = '/search'
// 店员手机端
export const clerkHome = '/clerk/home'
export const clerkApplicationPath = '/clerk/application'
export const clerkApplicationBasicPath = '/clerk/application/basic'
export const clerkApplicationWorkPath = '/clerk/application/work'
export const clerkApplicationFilesPath = '/clerk/application/files'
export const orderTakePath = "/order/take"

export const clerkPage = [
  clerkHome,
  clerkApplicationPath,
  clerkApplicationBasicPath,
  clerkApplicationFilesPath,
  orderTakePath
];
